...
def QuestionClassifier(scorer):
    examples = [
        {
            "input": "Ellie Goulding worked with what other writers on her third studio album, Delirium?",
            "output": "hard",
        },
        {
            "input": "Rostker v. Goldberg held that the practice of what way of filling armed forces vacancies was consitutional?",
            "output": "medium",
        },
        {
            "input": "does human hair stop squirrels",
            "output": "easy",
        },
        {
            "input": "Landolt C and snellen e acuity: differences in strabismus amblyopia?",
            "output": "medium",
        },
    ]

    system_prompt = (
        "You are a helpful assistant that classifies question difficulty for a question answering system. "
        "Your task is to categorize each question into one of three levels based on the reasoning complexity and domain difficulty:\n\n"
        "- 'easy': Simple fact-based questions that can be answered with a single sentence or fact. Typical of MS MARCO.\n"
        "- 'medium': Questions that involve some comparison, domain-specific knowledge (e.g., biomedical), or involve selecting between options. Typical of PubMedQA or HotpotQA comparison questions.\n"
        "- 'hard': Multi-hop reasoning questions that require combining information from multiple facts to answer. Typical of HotpotQA bridge questions.\n\n"
        "Respond with one of the following labels only: easy, medium, hard.\n\n"
    ...
    difficulty_level = scorer_prompt | scorer | StrOutputParser()
...
def run_rag_inference_pipeline(question_file: str, retriever, max_questions: int = 50):
    ...
    question_classifier = QuestionClassifier(scorer)
    lambda_set = []
    results = []

    for item in qa_pairs[:max_questions]:
        q = item["question"]
        qtype = question_classifier.invoke({"question": q})
        lamda = choose_lambda(qtype)
        lambda_set.append(lamda)
